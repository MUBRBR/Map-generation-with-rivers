...
def extend(v : Vert, e: f32, vNear: Vert, vFar: Vert, s: f32) : f32 =
    if  v.h < f32.min 0.0 e && 0.0 < vFar.h then
        ...
        between(e, min3 (v.h, vNear.h, vFar.h), nxt v.s)
    else -100.0
...
-- Epsilon multiplied by 1, sqrt(2), 2 or sqrt(8) will give one earlier step of subdivision.
-- See report for clarification.
def findSubTriangle (currentPosition : Position, curTri : Triangle, epsilon) : Triangle =
    loop curTri while distance(curTri.vert1, curTri.vert2) >= (1.42*epsilon) do
...
let closestMainTriangle (point : Position) (triangle1 : Triangle) (triangle2 : Triangle) : bool =
    let tri1v0pos = {x=triangle1.vert0.x, y=triangle1.vert0.y}
    let tri2v0pos = {x=triangle2.vert0.x, y=triangle2.vert0.y}
    let dv1 = distanceWithoutSqrt point tri1v0pos
    let dv2 = distanceWithoutSqrt point tri2v0pos
    in dv1 < dv2

def chooseMainTriangle(curPos, tri1, tri2) =
    if closestMainTriangle curPos tri1 tri2 then
        tri1
    else
        tri2
...
def main (resolution : i64) (zoomXStart : f32) (zoomYStart : f32) (viewDistance : f32) =
    ...
    let seed22 = mix (seed, 2.83492)
    let h01 = between(-0.2,0.7,seed01)
    let h0m1 = between(-0.6,-0.1,seed0m1)
    let h21 = between(-0.6,-0.1,seed21)
    let h22 = between(-0.6, 0.7, seed22)
    let upperLeftTriangle = 
        {vert0={x= -5.0, y= -5.0, s=seed01, h=h01}, 
        vert1={x= -5.0, y=5.0, s=seed21, h=h21}, 
        vert2={x= 5.0, y= -5.0, s=seed0m1, h=h0m1}, 
        e0=(-100), e1=(-100), e2=(-100)}
    let lowerRightTriangle = 
        {vert0={x= 5.0, y= 5.0, s=seed22, h=h22}, 
