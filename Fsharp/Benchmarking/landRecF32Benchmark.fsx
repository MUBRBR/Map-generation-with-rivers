...
let mutable width = 8192 // size of bitmap array
...
// e3 between v3 and v0
  ...
  then () // outside bounding box
    ...
    then // plot point
      ...
      (tria v3 v0 v1 e2 e5 e3 xmin xmax ymin ymax epsilon;
...
let main args =
  ...
  let mutable scale = (float32) 1.0
  let mutable i = 0
  let mutable outfile = "RecF32"
  ...
  tria {x=((float32)0.0); y=((float32)1.0); s=seed01; h=h01}
       {x=((float32)2.0); y=((float32)1.0); s=seed21; h=h21}
       {x=((float32)0.0); y= ((float32) -1.0); s=seed0m1; h=h0m1}
       None
       None
       None
       xmin xmax ymin ymax
